...
export default function FeePaymentScreen() {
  ...
  const startPayment = async () => {
    try {
      ...
      setCheckoutUrl(data.checkoutUrl); // ✅ from your backend
      setShowWebView(true);
    } catch (err) {
...
const styles = StyleSheet.create({
  ...
  amountLabel: { fontSize: 18, color: "#6b7280", marginBottom: 8 },
  amountValue: { fontSize: 36, fontWeight: "800", color: "#1e3a8a" },
  payButton: {
    width: "100%",
    marginTop: 30,
    borderRadius: 16,
  ...
  loadingText: { marginTop: 12, color: "#374151", fontSize: 16 },
});
